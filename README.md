...
- 持久化：浏览器 `localStorage`，键为 `ielts-sentence-lab-mvp-v1.1.0`。
- Listening：一个完整音频文件；每个 Sentence 保存 `startMs/endMs`，播放时只播放该时间片。
...
SentenceUnit
  id, materialId, orderIndex, textEn, textZh, audioKind,
  startMs, endMs, boundaryStatus
...
- Listening：`Riverside Visitor Centre`，12 句，完整测试音频 + 时间戳。
